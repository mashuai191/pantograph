...
#Dataset_Preparation
np.random.seed(2)
#获取night_frame文件夹下的所有图片
files=glob.glob('night_frame/*')
#总文件个数
print(len(files))
#选择有燃弧和没有燃弧的

have_files=[fn for fn in files if 'have' in fn]
no_files=[fn for fn in files if 'no' in fn]
print(len(have_files),len(no_files))

#选择训练样本
have_train=np.random.choice(have_files, size=np.ceil(0.8*(len(have_files))).astype(np.int), replace=False)
no_train=np.random.choice(no_files, size=np.ceil(0.8*(len(no_files))).astype(np.int), replace=False)
have_files=list(set(have_files) - set(have_train))
no_files=list(set(no_files) - set(no_train))

#验证集
have_val=np.random.choice(have_files, size=90, replace=False)
no_val=np.random.choice(no_files, size=90, replace=False)
have_files=list(set(have_files)-set(have_val))
no_files=list(set(no_files)-set(no_val))

#测试集
have_test=np.random.choice(have_files, size=90, replace=False)
no_test=np.random.choice(no_files, size=90, replace=False)
...
print(train_labels[5:10],train_labels_enc[5:10])
...
model.save('fine-tune vgg16 with img_aug.h5')


from PIL import Image
from keras.models import load_model
#matplotlib inline
finetune_vgg16_with_img_aug=load_model('fine-tune vgg16 with img_aug.h5')

num2class_label_transformer = lambda l: ['have' if x == 0 else 'no' for x in l]
class2num_label_transformer = lambda l: [0 if x == 'have' else 1 for x in l]
